"""File store abstraction for RAG Lite document storage.

Ships with ``LocalFileStore`` — files on the local filesystem at a
configurable root path.  The ``FileStore`` protocol enables EE to add
``MinioFileStore`` without changing consumers.
"""
