...
#: What a chunk is indexed by for keyword search: its context and its text.
_KEYWORD_TEXT = "'english', coalesce(context, '') || ' ' || chunk_text"
...
class RagLiteChunk(Base):
    ...
    #: What the chunk belongs to -- its document and section -- as a line
    #: of text, keyword-indexed with the chunk; ``chunk_text`` is what a
    #: search returns. Nothing writes it yet: a useful line (contextual
    #: retrieval) takes a model call per chunk, and RAGBench -- mostly one
    #: chunk per document, titles that are boilerplate -- could not show
    #: what it gains.
    context: Mapped[str | None] = mapped_column(Text, nullable=True)
    #: Keyword index for hybrid search, generated by the database.
    content_tsv = mapped_column(TSVECTOR, Computed(f"to_tsvector({_KEYWORD_TEXT})", persisted=True))
    #: How many distinct words ``content_tsv`` holds: BM25's document length,
    #: kept apart so the average over a collection reads a number per chunk,
    #: not every chunk's whole index entry.
    content_len: Mapped[int | None] = mapped_column(
        Integer, Computed(f"length(to_tsvector({_KEYWORD_TEXT}))", persisted=True),
    )
    ...
    __table_args__ = (
        ...
        Index("ix_rag_lite_chunks_content_tsv", "content_tsv", postgresql_using="gin"),
    )
