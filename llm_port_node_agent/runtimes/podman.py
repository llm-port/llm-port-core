"""Podman-backed implementation of :class:`ContainerRuntime`."""
...
class PodmanRuntime:
    """Execute container operations via the Podman CLI.

    Implements the :class:`ContainerRuntime` protocol using
    ``podman`` subprocess calls. Key differences from Docker:

    - GPU passthrough uses CDI (``--device nvidia.com/gpu=all``)
      instead of ``--gpus``.
    - The rootless socket path differs from Docker's.
    """
    ...
    async def _exec(
        ...
        *args: str,
        ...
        if self._socket_path:
            cmd.extend(["--url", f"unix://{self._socket_path}"])
        cmd.extend(args)
    ...
    async def load_image_tar(
        ...
        *,
        ...
        """Pipe an async byte-stream into ``podman load``."""
        cmd: list[str] = ["podman"]
        if self._socket_path:
            cmd.extend(["--url", f"unix://{self._socket_path}"])
        cmd.append("load")
        ...
        if proc.returncode != 0:
            ...
            raise ContainerRuntimeError(
                f"podman load failed (rc={proc.returncode}): {err_msg}"
            )

        result = stdout.decode("utf-8", "replace").strip()
        log.info("podman load: %d bytes → %s", total_bytes, result)
        return result
